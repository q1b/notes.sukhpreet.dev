...
const { class: className } = defineProps(['class', 'variant'])
</script>
<template>
  <button :class="cn(`inline-flex items-center gap-x-2 w-max rounded-lg px-2.5 py-1 shadow-sm border transition-colors ease-in
    bg-white text-slate-600 border-slate-200/75 
    ...
    active:bg-slate-100 active:text-slate-700 active:border-slate-300 
    ...
    [-webkit-tap-highlight-color:transparent;] select-none outline-none`, className)">
    <slot />
  </button>
</template>
